...
export default function EventCard({ event, timezone, onEdit, onViewLogs }) {
  return (
    <div className="event-card">
      ...
      <div className="event-meta" style={{margin:'16px 0 8px 0'}}>
        <div style={{display:'flex',alignItems:'center',marginBottom:6}}>
          <CalendarIcon />
          <span style={{fontWeight:500}}>Start:</span>
          <span style={{marginLeft:6}}>{fmt(event.start, timezone)}</span>
          <ClockIcon />
        ...
        <div style={{display:'flex',alignItems:'center'}}>
          ...
          <span style={{marginLeft:6}}>{fmt(event.end, timezone)}</span>
          <ClockIcon />
          <span>{fmtTime(event.end, timezone)}</span>
        </div>
      </div>
      <div className="divider" />
      <div className="event-meta" style={{margin:'16px 0 8px 0',color:'#64748b',fontSize:14}}>
        <div>Created {fmt(event.createdAt, timezone)} at {fmtTime(event.createdAt, timezone)}</div>
        <div>Updated {fmt(event.updatedAt, timezone)} at {fmtTime(event.updatedAt, timezone)}</div>
      </div>
      <div className="divider" />
